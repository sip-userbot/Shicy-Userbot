...
while 0 < 6:
    _DEVS = get(
        "https://raw.githubusercontent.com/sip-userbot/Reforestation/master/DEVS.json"
    )
...
UPSTREAM_REPO_URL = os.environ.get("UPSTREAM_REPO_URL", "https://github.com/sip-userbot/Shicy-Userbot.git")
...
ALIVE_TEKS_CUSTOM = os.environ.get(
    "ALIVE_TEKS_CUSTOM",
    "𝙷𝙴𝚈, 𝚂𝙰𝚈𝙰 𝙿𝙴𝙽𝙶𝙶𝚄𝙽𝙰 Shicy-Userbot")

# Default .alive name
ALIVE_NAME = os.environ.get("ALIVE_NAME", "ShicyXd")
...
ALIVE_LOGO = (os.environ.get("ALIVE_LOGO")
              or "https://telegra.ph/file/83f249f03c3145c9089b3.jpg")

INLINE_PIC = (os.environ.get("INLINE_PIC")
              or "https://telegra.ph/file/225f4098de5a4367e4a76.jpg")

# Picture For VCPLUGIN
PLAY_PIC = (os.environ.get("PLAY_PIC")
            or "https://telegra.ph/file/87781ec43b1a6b38cabfe.jpg")
...
while 0 < 6:
    _BLACKLIST = get(
        "https://raw.githubusercontent.com/sip-userbot/Reforestation/master/Shicyblacklist.json"
    )
    if _BLACKLIST.status_code != 200:
        ...
        blacklistShicy = []
        break
    blacklistShicy = _BLACKLIST.json()
    break

del _BLACKLIST

ch = str(b64decode("QFNoaWN5eEMwZA=="))[2:15]
gc = str(b64decode("QFNoaWN5eVhDb2Rl"))[2:17]

while 0 < 6:
    _WHITELIST = get(
        "https://raw.githubusercontent.com/sip-userbot/Reforestation/master/whitelist.json"
    )
...
async def update_restart_msg(chat_id, msg_id):
    message = (
        f"**Shicy-UserBot v`{BOT_VER}` is back up and running!**\n\n"
        f"**Telethon:** `{vsc}`\n"
...
try:
    from ShicyXd.modules.sql_helper.globals import delgvar, gvarstatus
...
with bot:
    try:
        ...
        from ShicyXd.modules.sql_helper.bot_blacklists import check_is_black_list
        from ShicyXd.modules.sql_helper.bot_pms_sql import add_user_to_db, get_user_id
        from ShicyXd.modules.sql_helper.globals import addgvar, delgvar, gvarstatus
        from ShicyXd.shicy import ShicyDB, HOSTED_ON, reply_id
        from Stringchiy import get_languages, get_string, language
        ...
        logo = ALIVE_LOGO
        logochiy = random.choice(
                [
                    "https://telegra.ph/file/83f249f03c3145c9089b3.jpg",
                    "https://telegra.ph/file/225f4098de5a4367e4a76.jpg",
                    "https://telegra.ph/file/55d4f8d54263f20fbcb31.jpg",
                    "https://telegra.ph/file/87781ec43b1a6b38cabfe.jpg",
                ]
        )
        cmd = CMD_HANDLER
        ...
        main_help_button = [
            [
                Button.inline(get_string("help_3"), data="konten_chiy"),
                Button.inline(get_string("help_4"), data="inline_chiy"),
            ],
            [
                Button.inline(get_string("help_2"), data="reopen"),
            ],
            [
                Button.inline(get_string("help_6"), data="chiy_langs"),
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                ...
                text = f"**✨ Shicy-ᴜsᴇʀʙᴏᴛ ɪɴʟɪɴᴇ ᴍᴇɴᴜ ✨**\n\n⍟ **ʙᴀsᴇ ᴏɴ :** {adB.name}\n⍟ **ᴅᴇᴘʟᴏʏ :** •[{HOSTED_ON}]•\n⍟ **ᴏᴡɴᴇʀ** {user.first_name}\n⍟ **ᴊᴜᴍʟᴀʜ :** {len(dugmeler)} **Modules**"
                await event.edit(
                    text,
                    file=logochiy,
                    buttons=buttons,
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query.startswith(
                ...
                result = builder.article(
                    title="Repository",
                    description="Repository Shicy - Userbot",
                    ...
                    thumb=InputWebDocument(
                        ...
                        "image/jpeg",
                        []),
                    text="**Shicy-Userbot**\n➖➖➖➖➖➖➖➖➖➖\n✧  **ʀᴇᴘᴏ :** [SHICY](https://t.me/CarddSamss)\n✧ **sᴜᴘᴘᴏʀᴛ :** @ShicyyXCode\n✧ **ʀᴇᴘᴏsɪᴛᴏʀʏ :** [Shicy-Userbot](https://github.com/sip-userbot/Shicy-Userbot)\n➖➖➖➖➖➖➖➖➖➖",
                    buttons=[
                        [
                            custom.Button.url(
                                "ɢʀᴏᴜᴘ",
                            ...
                            custom.Button.url(
                                "ʀᴇᴘᴏ",
                                "https://github.com/sip-userbot/Shicy-Userbot"),
                        ],
                    ],
                    link_preview=False,
                )
                ...
                result = builder.article(
                    title="String",
                    description="String Shicy - Userbot",
                    url="https://t.me/ShicyyXCode",
                    thumb=InputWebDocument(
                        logochiy,
                        0,
                    ...
                    buttons=[
                        [
                            custom.Button.url(
                                "Bᴏᴛ Sᴛʀɪɴɢ",
                                url="https://t.me/ShicyStringRobot?start="),
                            custom.Button.url(
                                "Sᴛʀɪɴɢ Rᴇᴘʟɪᴛ",
                                url="https://repl.it/@ShicyXd/ShicyString?lite=1&outputonly=1"),
                        ],
                        [
                ...
                tutud = [
                    Button.inline(
                        f"{languages[chiy]['asli']} [{chiy.lower()}]",
                        data=f"set_{chiy}",
                    )
                    for chiy in languages
                ]
                buttons = list(zip(tutud[::2], tutud[1::2]))
                if len(tutud) % 2 == 1:
                    buttons.append((tutud[-1],))
                buttons.append([custom.Button.inline("ʙᴀᴄᴋ", data="chiy_close")])
                result = builder.article(
                    title="Lang",
                    description="Lang Shicy - Userbot",
                    url="https://t.me/ShicyyXCode",
                    thumb=InputWebDocument(
                        logochiy,
                        0,
                ...
                result = builder.article(
                    title="✨  Shicy-Usᴇʀʙᴏᴛ ✨",
                    description="Shicy - Userbot | Telethon",
                    url="https://t.me/ShicyyXCode",
                    thumb=InputWebDocument(
                        INLINE_PIC,
                        0,
        ...
        async def gback_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:  # @Shicy-Userbot
                # https://t.me/TelethonChat/115200
                text = (
                    f"**✨  Shicy-ᴜsᴇʀʙᴏᴛ ɪɴʟɪɴᴇ ᴍᴇɴᴜ ✨**\n\n✧ **ᴏᴡɴᴇʀ :** [{user.first_name}](tg://user?id={user.id})\n✧ **ᴊᴜᴍʟᴀʜ :** {len(dugmeler)} **Modules**")
                await event.edit(
                    text,
                    file=logochiy,
                    link_preview=True,
                ...
                if languages:
                    try:
                        ...
                        await event.edit(
                            ...
                            file=logochiy,
                            link_preview=True,
                            buttons=[Button.inline("ʙᴀᴄᴋ", data="chiy_close")]
                        )
                    except Exception:
                        pass
        ...
        @tgbot.on(events.CallbackQuery(data=b"inline_chiy"))
...
""")
                await event.edit(
                    ...
                    file=logochiy,
                    link_preview=True,
                    buttons=[Button.inline("ʙᴀᴄᴋ", data="inline_chiy")])
            else:
                reply_pop_up_alert = f"❌ DISCLAIMER ❌\n\nAnda Tidak Mempunyai Hak Untuk Menekan Tombol Button Ini"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
...
""")
        ...
        @tgbot.on(events.CallbackQuery(data=b"konten_chiy"))
        async def about(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                await event.edit(f"""
...
""")
                await event.edit(
                    ...
                    file=logochiy,
                    link_preview=True,
                    buttons=[Button.inline("ʙᴀᴄᴋ", data="konten_chiy")])
            else:
                reply_pop_up_alert = f"❌ DISCLAIMER ❌\n\nAnda Tidak Mempunyai Hak Untuk Menekan Tombol Button Ini"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                text = (
                    f"""
✘ **Perintah yang tersedia di chiy bokep** ✘
...
""")
        ...
        async def close(event):
            ...
            await event.edit("**ᴍᴇɴᴜ ᴅɪᴛᴜᴛᴜᴘ**", file=logochiy, buttons=buttons)

        @tgbot.on(
            events.callbackquery.CallbackQuery(  # pylint:disable=E0602
                data=re.compile(rb"chiy_close")
            )
        )
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                await event.edit(get_string("lang_3"), file=logochiy)
        ...
        LOGS.info(
            f"KALO BOT LU NGECRASH, KLIK SAVE YANG DI POJOK KANAN BAWAH DAN KIRIM KE @ShicyyXCode » TAG @admin » Info By: Shicy-Userbot {BOT_VER}")
