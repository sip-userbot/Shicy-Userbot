...
from ShicyXd import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME
from ShicyXd.shicy import shicy_cmd, eor, time_formatter
...
@shicy_cmd(pattern="sleep ([0-9]+)$", allow_sudo=False)
...
@shicy_cmd(pattern="shutdown$", allow_sudo=False)
...
@shicy_cmd(pattern="restart$", allow_sudo=False)
async def restart_bot(event):
    ...
    args = [sys.executable, "-m", "ShicyXd"]
    execle(sys.executable, *args, environ)


@shicy_cmd(pattern="readme$")
...
@shicy_cmd(pattern="repo$")
async def repo_is_here(event):
    shicy = await eor(event, "🔥")
    sleep(3)
    await shicy.edit(get_string("repo_1")
    )


@shicy_cmd(pattern="raw$")
async def raw(event):
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}sleep`\
        \n  »  **Kegunaan : **Biarkan shicy-Userbot tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository shicy-Userbot`\
        \n\n  »  **Perintah :** `{cmd}repo`\
        \n  »  **Kegunaan : **Menampilan link Repository shicy-Userbot\
        \n\n  »  **Perintah :** `{cmd}string`\
        \n  »  **Kegunaan : **Menampilan link String shicy-Userbot\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart shicy-Userbot`\
