...
from ShicyXd.shicy import shicy_cmd, runcmd
from Stringchiy import get_string


@shicy_cmd(pattern="convert ?(foto|audio|gif|voice|photo|mp3)? ?(.*)")
async def cevir(event):
    botchiy = event.pattern_match.group(1)
    try:
        if len(botchiy) < 1:
...
async def makevoice(event):
    ...
    await runcmd(
        f"ffmpeg -i '{file}' -map 0:a -codec:a libopus -b:a 100k -vbr on chiy.opus"
    )
    await event.client.send_message(
        event.chat_id, file="chiy.opus", force_document=False, reply_to=msg
    )
    await xxnx.delete()
    os.remove(file)
    os.remove("chiy.opus")
