...
from ShicyXd.modules.chiyping import absen
from ShicyXd.shicy import shicy_cmd, get_user_from_event
from Stringchiy import get_string


@shicy_cmd(pattern="userid$")
...
@shicy_cmd(pattern="inviteall ?(.*)")
async def get_users(event):
    shicy_ = event.text[11:]
    chat_shicy = shicy_.lower()
    restricted = ["@ShicyXdSupport", "@ShicyXdsupport"]
    if chat_shicy in restricted:
    ...
    shicy = await eor(event, get_string("inal_5").format(shicy_))
    shicyuserbot = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await shicy.edit(get_string("inal_4")
                                )
    s = 0
    f = 0
    error = "None"
    await shicy.edit(get_string("inal_6"))
    async for user in event.client.iter_participants(shicyuserbot.full_chat.id):
        try:
            await event.client(InviteToChannelRequest(channel=chat, users=[user.id]))
            s += 1
            await shicy.edit(get_string("inal_8").format(s, f, error)
                             )
        except Exception as e:
            error = str(e)
            f += 1
    return await shicy.edit(get_string("inal_7").format(s, f)
                            )
...
CMD_HELP.update(
    {
        ...
        \n  •  **Contoh : **{cmd}link @ShicyXd Ganteng\
    "
    }
)
