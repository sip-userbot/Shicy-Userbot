...
from ShicyXd.shicy import shicy_cmd, edit_delete, edit_or_reply
...
@shicy_cmd(pattern=r"font (monospace|smallcap|outline|bold|bolditalic|script|bubbles|blackbubbles) (.*)")
async def font_yins(shicy):
    if shicy.pattern_match.group(1) == "monospace":
        xd = monospace
    if shicy.pattern_match.group(1) == "smallcap":
        xd = smallcap
    if shicy.pattern_match.group(1) == "outline":
        xd = outline
    if shicy.pattern_match.group(1) == "bold":
        xd = bold
    if shicy.pattern_match.group(1) == "bolditalic":
        xd = bolditalic
    if shicy.pattern_match.group(1) == "script":
        xd = script
    if shicy.pattern_match.group(1) == "bubbles":
        xd = bubbles
    if shicy.pattern_match.group(1) == "blackbubbles":
        xd = blackbubbles
    kontol = xd(shicy.pattern_match.group(2))
    if not kontol:
        return await edit_delete(shicy, "`Ngetik Yang Bener Bego...`")

    await shicy.edit(f"{kontol}")
    await shicy.reply("**𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚𝙙 𝘽𝙮 :** ✧ 𝙰𝚈𝙸𝙸𝙽-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧")
...
@shicy_cmd(pattern="font(.*)(|$)")
async def _(shicy):
    input = shicy.pattern_match.group(1).strip()
    reply = await shicy.get_reply_message()
    reply_to_id = shicy.message
    if shicy.reply_to_msg_id:
        reply_to_id = await shicy.get_reply_message()
    ...
    if not reply:
        try:
            ...
            return await edit_delete(shicy, reply_to_id)
    elif not input:
        return await edit_delete(shicy, "`Kasih Font Dong Bego :/`")

    else:
        font = input
        text = reply.message
    if not font:
        return await edit_delete(shicy, f"`{font} Tidak Ada Dalam Daftar Font Kentod...`", time=5)
    if font == "smallcap":
    ...
    await edit_or_reply(shicy, yins)
    await shicy.reply("**𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚𝙙 𝘽𝙮 :** ✧ 𝙰𝚈𝙸𝙸𝙽-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧")
    if not yins:
        await shicy.reply("Ketik Yang Bener Bego!!!")
...
@shicy_cmd(pattern="lf(.*)(|$)")
async def fonts(yins):
    shicy = await edit_or_reply(yins,
                                "**»» ᴅᴀғᴛᴀʀ ғᴏɴᴛs ««**\n"
