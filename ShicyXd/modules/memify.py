...
from ShicyXd.shicy import edit_delete, edit_or_reply, shicy_cmd, runcmd, take_screen_shot


@shicy_cmd(pattern="mmf (.*)")
async def memify(event):
    reply_msg = await event.get_reply_message()
    input_str = event.pattern_match.group(1)
    xx = await edit_or_reply(event, "`Sedang Memperoses Tod...`")
    if not reply_msg:
        return await edit_delete(xx, "**Balas ke pesan yang berisi media goblog!**")
...
async def add_text_img(image_path, text):
    ...
    font = ImageFont.truetype(
        font="ShicyXd/shicy/styles/FontLord.ttf",
