...
from ShicyXd import S_PACK_NAME as custompack
from ShicyXd import tgbot
from ShicyXd.modules.sql_helper.globals import addgvar, gvarstatus
from ShicyXd.shicy import shicy_cmd, eod, eor
from ShicyXd.shicy.misc import animator, create_quotly
...
KANGING_STR = [
    ...
    "Gw ambil stiker lo dulu yee bosss"
]


@shicy_cmd(pattern="(?:tikel|kang)\\s?(.)?")
async def kang(args):
...
@shicy_cmd(pattern="pkang(?:\\s|$)([\\s\\S]*)")
async def _(event):
    xnxx = await eor(event, f"`{choice(KANGING_STR)}`")
    reply = await event.get_reply_message()
    query = event.text[7:]
    shicyBot = await tgbot.get_me()
    BOT_USERNAME = shicyBot.username
    bot_ = BOT_USERNAME
    bot_un = bot_.replace("@", "")
        ...
        try:
            create_st = await tgbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
                    title=pname,
                    short_name=f"shicy_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
@shicy_cmd(pattern="stickers ?([\\s\\S]*)")
