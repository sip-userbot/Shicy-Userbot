...
async def _(event):
    ...
    if len(final_output) > 4096:
        chiy = final_output.replace(
            ...
            "__",
            "")
        with io.BytesIO(str.encode(chiy)) as out_file:
...
@shicy_cmd(pattern="exec(?: |$|\n)([\\s\\S]*)")
async def run(event):
    code = event.pattern_match.group(1)
    if not code:
        return await eor(event, get_string("devs_1"))
    if code in ("ShicyXd.session", "config.env"):
        return await eor(event, get_string("devs_2"))
