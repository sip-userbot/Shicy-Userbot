...
from ShicyXd import ALIVE_EMOJI, ALIVE_LOGO, ALIVE_TEKS_CUSTOM, BOT_VER, CHANNEL
from ShicyXd import CMD_HANDLER as cmd
from ShicyXd import CMD_HELP, GROUP, StartTime
from ShicyXd.shicy import __version__ as py_ver
...
async def psu(event):
    ...
    help_string += f"**Pʏ-Shicy :** `{py_ver}`\n"
    help_string += f"**Shicy Vᴇʀsɪᴏɴ :** `{shicy_version} [{HOSTED_ON}]`"
...
@shicy_cmd(pattern="(?:alive|chiyon)\\s?(.)?")
async def amireallyalive(alive):
    ...
    output = (
        f"**Tʜᴇ [Shicy-Userbot](https://github.com/sip-Userbot/Shicy-Userbot)**\n\n"
        f"**{alive_text}**\n\n"
        f"╭✠╼━━━━━━━━━━━━━━━✠╮\n"
        ...
        f"{emoji} **Pʏ-Shicy Vᴇʀsɪᴏɴ :** `{py_ver}`\n"
        f"{emoji} **Shicy Vᴇʀsɪᴏɴ :** `{shicy_version}` [{HOSTED_ON}]\n"
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}alive` atau `{cmd}chiyon`\
