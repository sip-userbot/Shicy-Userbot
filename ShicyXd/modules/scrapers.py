...
from ShicyXd.modules.upload_download import get_video_thumb
...
from ShicyXd.shicy.FastTelethon import upload_file
...
@shicy_cmd(pattern="currency ([\\d\\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@shicy_cmd(pattern="google ([\\s\\S]*)")
async def gsearch(q_event):
    chiy = await eor(q_event, get_string("com_1"))
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await eod(chiy, get_string("error_1").format(e), time=10)
    msg = ""
    for i in range(lim):
...
@shicy_cmd(pattern=r"lang (tr|tts) (.*)")
...
@shicy_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
...
async def kbg(remob):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "shicy_bg.png"
...
@shicy_cmd(pattern=r"makeqr(?: |$)([\s\S]*)")
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example :** `{cmd}google -l4 ShicyXd` atau `{cmd}google ShicyXd`\
    "
    }
)
