...
from ShicyXd.modules.sql_helper.bot_starters import (
    add_starter_to_db,
    get_all_starters,
    get_starter_details,
)
from ShicyXd.modules.sql_helper.globals import gvarstatus
from ShicyXd.shicy import _format, asst_cmd, callback, reply_id
...
async def pmbot(event):
    ...
    botusername = shicyUBOT.username
    if event.query.user_id == OWNER_ID:
...
async def strone(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"**Silahkan Kirimkan {var} Telethon anda dari @shicyStringRobot**\n\nGunakan /cancel untuk membatalkan."
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
    ...
    if chat.id != OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**👋 Hai** {mention}**!**\
                        ...
                        \n\n**Powered by** [UserBot](https://github.com/ShicyXd/shicy-Userbot)"
        buttons = [
            (
