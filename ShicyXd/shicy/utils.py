...
async def autopilot():
    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="Shicy-Userbot Lᴏɢs",
                about="» Group log Created by: Shicy-Userbot\n\n» Support : @ShicyyXCode\n» Support: @ShicyyxC0d",
                megagroup=True,
            ),
        )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/225f4098de5a4367e4a76.jpg", "photochiy.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Shicy{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Shicy{(str(who.id))[6:]}{str(ran)}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ShicyXd/modules/{shortname}.py")
        name = "ShicyXd.modules.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["ShicyXd.modules." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ShicyXd/modules/assistant/{shortname}.py")
        name = "ShicyXd.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["ShicyXd.modules.assistant" + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"ShicyXd.modules.{shortname}"
