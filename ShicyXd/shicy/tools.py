...
from ShicyXd import LOGS, SUDO_USERS, bot
from ShicyXd.shicy.format import md_to_text, paste_message
...
async def extract_time(chiy, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]
        if not time_num.isdigit():
            await chiy.edit("Jumlah waktu yang ditentukan tidak valid.")
            return None
        if unit == "s":
...
def post_to_telegraph(title, html_format_content):
    ...
    auth_name = "Shicy-Userbot"
    auth_url = "https://github.com/sip-userbot/Shicy-Userbot"
