...
from ShicyXd import DEVS, TEMP_DOWNLOAD_DIRECTORY
from ShicyXd.shicy import runcmd
...
async def animator(media, mainevent, textevent):
    ...
    Shicy = await mainevent.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
    await textevent.edit("`Converting...`")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {shicy} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an Video.webm"
    )
    os.remove(Shicy)
    vid = "Video.webm"
    return vid
