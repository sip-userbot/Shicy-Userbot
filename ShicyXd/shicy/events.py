...
async def get_user_from_event(
    event, chiyevent=None, secondgroup=None, nogroup=False, noedits=False
):
    if chiyevent is None:
        chiyevent = event
    if nogroup is False:
        if secondgroup:
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.sender_id is None:
                if not noedits:
                    await edit_delete(
                        chiyevent, "**ERROR: Dia adalah anonymous admin!**", 60
                    )
                return None, None
...
async def checking(client):
    ...
    checker = str(pybase64.b64decode("QFNoaWN5eVhDb2Rl"))[2:17]
    checkxd = str(pybase64.b64decode("QEFrdGlma2FubmJvdA=="))[2:13]
    checksbt = str(pybase64.b64decode("QE11dHVhbGFuc2hpY3k="))[2:17]
    if client:
        try:
